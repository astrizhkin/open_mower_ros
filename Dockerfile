...
COPY --link --from=fetch /opt/open_mower_ros/src/lib/Slic3r /opt/slic3r_workspace/src
...
RUN bash -c "source /opt/ros/$ROS_DISTRO/setup.bash && source /opt/slic3r_workspace/devel/setup.bash && catkin_make -DCMAKE_INSTALL_PREFIX=/opt/prebuilt/slic3r install"
...
COPY --link --from=slic3r /opt/prebuilt/slic3r /opt/prebuilt/slic3r
...
RUN rm -rf /opt/open_mower_ros/src/lib/Slic3r /apt-install_list
