...
void updateEmergencyBits() {
  ...
  if ((~curr_kill_switch_em_bits & prev_kill_switch_em_bits) && emergency_high_level_bits) {
...
void onFrontOdomReceived(const nav_msgs::Odometry::ConstPtr &msg) {
  // ROS_INFO_STREAM("[mower_comms] Got front driver state: "<< +msg->state.connection_state);
  last_front_odom = *msg;
}
#endif
    ...
    while (*contact_token != 0) {
      ...
      if(c>='0' && c<='9') {
        ...
        switch (std::toupper(c)) {
          ...
          case 'L': mode = ContactMode::MONITOR;
                   latch_contact_mask |= (1 << contact_idx); break;
