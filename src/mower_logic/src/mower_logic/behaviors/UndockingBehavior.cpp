...
Behavior *UndockingBehavior::execute() {
  ...
  if (config.undock_fixed_angle) {
    ...
    ROS_INFO_STREAM("[UndockingBehavior] Fixed angle undock: " << config.undock_angle);
  } else {
    // seed based on first undock time rather than boot so should be ok even without RTC
    if (rng_seeding_required) {
      srand(ros::Time::now().toSec());
      ROS_INFO_STREAM("[UndockingBehavior] Random angle undock: Seeded rand()");
    ...
    ROS_INFO_STREAM("[UndockingBehavior] Random angle undock: " << random_angle_deg);
